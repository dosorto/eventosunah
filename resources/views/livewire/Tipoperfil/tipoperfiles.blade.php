    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500">
        ...
        @foreach($tipoperfiles as $tipoperfil)
                    <tr class="bg-white border-b hover:bg-gray-50">
                        <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">{{ $tipoperfil->id }}</th>
                        <td class="px-6 py-4">{{ $tipoperfil->tipoperfil }}</td>
                        <td class="px-6 py-4">
                        <button wire:click="edit({{ $tipoperfil->id }})" class="font-medium text-blue-600 hover:underline">Editar</button>
                        <button wire:click="delete({{ $tipoperfil->id }})" class="font-medium text-red-600 hover:underline">Borrar</button>
                        </td>
                    </tr>
                    @endforeach
