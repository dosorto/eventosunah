        ...
        <div class="">
            ...
            <div class="relative overflow-x-auto  sm:rounded-lg  dark:bg-gray-800">
                ...
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-600 bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex space-x-2">
                                            <button wire:click="edit({{ $tipoperfil->id }})"
                                                class="px-3 py-2 text-sm font-medium text-white inline-flex items-center bg-yellow-500 hover:bg-yellow-600 focus:ring-4 focus:outline-none focus:ring-yellow-300 rounded-lg text-center dark:bg-yellow-500 dark:hover:bg-yellow-600 dark:focus:ring-yellow-800">
                                                <svg class="w-6 h-6 text-white dark:text-white" aria-hidden="true"
                                                    xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none"
                                                    viewBox="0 0 24 24">
                                                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                                            ...
                                            <button wire:click="confirmDelete({{ $tipoperfil->id }})"
                                                ...
                                                <svg class="w-6 h-6 text-white dark:text-white" aria-hidden="true"
                                                    ...
                                                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                                                        stroke-width="2"
                                                        d="M5 7h14M10 11v6m4-6v6M4 7l1 12a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1L20 7M9 7V6a3 3 0 0 1 6 0v1" />
                                                </svg>
                                                Eliminar
                                            </button>
                                        </div>
                    ...
                    <div class="mt-4">{{ $tipoperfiles->links() }}</div>
                </div>
            </div>
        </div>
        <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center z-50 sm:block sm:p-0">
            <x-confirmation-modal wire:model="confirmingDelete">
                @if ($errorMessage)
                    <x-slot name="title">Error</x-slot>
                    <x-slot name="content">{{ $errorMessage }}</x-slot>
                    <x-slot name="footer">
                ...
                @else
                    ...
                    <x-slot name="content">¿Está seguro que desea eliminar este tipo de perfil?</x-slot>
                    <x-slot name="footer">
                        <x-secondary-button wire:click="$toggle('confirmingDelete')" wire:loading.attr="disabled">
                            Cancelar
                        </x-secondary-button>
